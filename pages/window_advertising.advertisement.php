...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    foreach (rex_clang::getAll() as $rex_clang) {
        if (!$advertisement instanceof TobiasKrais\D2UImmo\Advertisement) {
            $advertisement = new TobiasKrais\D2UImmo\Advertisement($ad_id, $rex_clang->getId());
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $advertisement = new TobiasKrais\D2UImmo\Advertisement($ad_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
elseif ('changestatus' === $func) {
    $advertisement = new TobiasKrais\D2UImmo\Advertisement($entry_id, (int) rex_config::get('d2u_helper', 'default_lang'));
...
elseif ('priority_down' === $func || 'priority_up' === $func) {
    ...
    if ('priority_down' === $func) {
        ...
        $advertisement->save();
    } elseif ($advertisement->priority > 1) {
        --$advertisement->priority;
        $advertisement->save();
    }
    ...
    <form action="<?= BackendHelper::getCurrentBackendPage([], ['message', 'message_type']) ?>" method="post">
		...
		<div class="panel panel-edit">
			...
			<div class="panel-body">
				...
				<?php
                    foreach (rex_clang::getAll() as $rex_clang) {
                        $advertisement = new TobiasKrais\D2UImmo\Advertisement($entry_id, $rex_clang->getId());
...
if ('' === $func) {
    $query = 'SELECT advertisements.ad_id, title, priority, online_status, '
        . '(SELECT MAX(priority) FROM '. rex::getTablePrefix() .'d2u_immo_window_advertising) AS max_priority '
    ...
    if (rex::getUser() instanceof rex_user && (rex::getUser()->isAdmin() || rex::getUser()->hasPerm('d2u_immo[edit_data]'))) {
        $list->addColumn(rex_i18n::msg('status_online'), '<a class="rex-###online_status###" href="' . BackendHelper::getCurrentBackendPage(['func' => 'changestatus'], [], true) . '&entry_id=###ad_id###"><i class="rex-icon rex-icon-###online_status###"></i> ###online_status###</a>');
    ...
    $list->setColumnFormat(rex_i18n::msg('d2u_helper_open_frontend'), 'custom', static function ($params) {
        ...
        return BackendHelper::getFrontendLinkButton((new \TobiasKrais\D2UImmo\Advertisement((int) $listParams->getValue('ad_id'), (int) rex_config::get('d2u_helper', 'default_lang')))->getUrl());
    });
