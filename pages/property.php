...
if (intval(filter_input(INPUT_POST, "btn_save")) === 1 || intval(filter_input(INPUT_POST, "btn_apply")) === 1) {
	...
	foreach(rex_clang::getAll() as $rex_clang) {
		if($property === false) {
			...
			if(isset($form['category_id']) && $form['category_id'] > 0) {
				$property->category = new D2U_Immo\Category($form['category_id'], intval(rex_config::get("d2u_helper", "default_lang")));
			}
	...
	if(intval(filter_input(INPUT_POST, "btn_apply", FILTER_VALIDATE_INT)) === 1 &&$property !== false) {
...
// Delete
	...
	$property = new D2U_Immo\Property($property_id, intval(rex_config::get("d2u_helper", "default_lang")));
...
// Change online status of machine
	...
	$property = new D2U_Immo\Property($entry_id, intval(rex_config::get("d2u_helper", "default_lang")));
			...
			<div class="panel-body">
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
							$options_categories = [];
							foreach(D2U_Immo\Category::getAll(intval(rex_config::get("d2u_helper", "default_lang"))) as $category) {
								if($category->name != "") {
