(function() {
	function updatePrintModalTarget(trigger) {
		var modalSelector = trigger.getAttribute("data-target") || trigger.getAttribute("data-bs-target");
		if (!modalSelector) {
			return;
		}
	...
	document.addEventListener("click", function(event) {
		var lightboxLink = event.target.closest("[data-d2u-immo-ekko-lightbox]");
		if (lightboxLink && typeof jQuery !== "undefined" && typeof jQuery.fn.ekkoLightbox === "function") {
			event.preventDefault();
			jQuery(lightboxLink).ekkoLightbox({ alwaysShowClose: true });
			return;
		}
	...
	if (typeof jQuery !== "undefined") {
		jQuery(function() {
			...
			if (hash) {
				jQuery(".d2u-immo-detail-bs4 ul.nav a[href='" + hash + "']").tab("show");
			}
			jQuery(".d2u-immo-detail-bs4 a[data-toggle='tab']").on("shown.bs.tab", function(e) {
				if (jQuery(e.target).attr("href") === "#tab_map") {
					invalidateMap();
				}
			});
